...
import org.folio.rs.error.TenantNotInitializedException;
...
public class KafkaErrorHandler implements KafkaListenerErrorHandler {
  ...
  private boolean isTenantNotInitializedYet(Message<?> message, ListenerExecutionFailedException exception) {
    ...
    return getThrowableList(exception).stream()
      .anyMatch(SQLGrammarException.class::isInstance) && isSupportedMessageType(message);
  }

  @SuppressWarnings("unchecked")
  private String[] getTenantNames(Message<?> message) {
    var consumerRecords = (List<ConsumerRecord<String, DomainEvent>>) message.getPayload();
    return consumerRecords.stream()
      .map(ConsumerRecord::value)
      .map(DomainEvent::getTenant)
      .distinct()
      .toArray(String[]::new);
  }
  ...
  private boolean isSupportedMessageType(Message<?> message) {
    ...
    return !payloads.isEmpty() && payloads.get(0) instanceof ConsumerRecord
        && ((ConsumerRecord) payloads.get(0)).value() instanceof DomainEvent;
  }
}
