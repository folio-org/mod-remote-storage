...
@Mapper(componentModel = "spring", nullValueCheckStrategy = NullValueCheckStrategy.ALWAYS) public interface ConfigurationsMapper {
  ...
  @Mappings({ @Mapping(target = "id", expression = "java(org.folio.rs.util.MapperUtils.uuidToStringSafe(configuration.getId()))"),
    ...
    @Mapping(target = "metadata.updatedByUsername", source = "updatedByUsername") }) StorageConfiguration mapEntityToDto(
  ...
  @Mappings({
    ...
    @Mapping(target = "updatedByUserId", expression = "java(storageConfiguration.getMetadata() == null ? null : org.folio.rs.util.MapperUtils.stringToUUIDSafe(storageConfiguration.getMetadata().getUpdatedByUserId()))") }) @InheritInverseConfiguration Configuration mapDtoToEntity(
    StorageConfiguration storageConfiguration);

  @Mappings({}) List<StorageConfiguration> mapEntitiesToDtos(Iterable<Configuration> remoteStorageConfigurationList);
